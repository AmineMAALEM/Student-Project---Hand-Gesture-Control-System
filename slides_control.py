...
while True:
    ...
    if hands and buttonPressed is False:  # If hand is detected
        ...
        #xVal = int(np.interp(lmList[8][0], [width // 2, width // 2 +200], [0, width]))
        xVal = int(np.interp(lmList[8][0], [200, width // 2], [0, width]))
        ...
        if fingers == [1, 1, 1, 1, 1] or fingers == [0, 1, 1, 1, 1]:# tous les doigts pour supprimer 
            if annotations:
                annotations.pop(-1)
                annotationNumber -= 1
                buttonPressed = True
